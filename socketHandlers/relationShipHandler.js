...
module.exports = (io, socket) => {
  ...
  socket.on(
    ...
    errorHandler(socket, async (data, callback) => {
      ...
      socket.to(to).emit("new_friend_req", {
        message: "New Friend request received",
        newFriendRequest: res,
      });

      socket.to(from).emit("send_req_ret", {
  ...
  socket.on(
    ...
    errorHandler(socket, async (data) => {
      ...
      socket.to(request.recipient.id).emit("friend_req_decline_ret", {
        message: "Decline friend request successful",
      });

      socket.to(request.senderId).emit("friend_req_decline_ret", {
  ...
  socket.on(
    ...
    errorHandler(socket, async (data) => {
      ...
      socket.to(userId).emit("withdraw_friend_req_ret", {
